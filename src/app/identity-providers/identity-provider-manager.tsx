...
type Mode = { kind: "list" } | { kind: "create" } | { kind: "edit"; provider: IdpProviderPublic };
...
export function IdentityProviderManager({ providers }: { providers: IdpProviderPublic[] }) {
  const [mode, setMode] = useState<Mode>({ kind: "list" });

  if (mode.kind !== "list") {
    return (
      <ProviderForm
        initial={mode.kind === "edit" ? mode.provider : undefined}
  ...
  return (
    <div>
      <div className="flex items-center justify-between border-b border-white/5 px-5 py-3">
        ...
        <Button onClick={() => setMode({ kind: "create" })} size="sm" variant="secondary">
      ...
      {providers.length === 0 ? (
        <div className="flex flex-col items-center gap-3 py-10 text-center">
          <p className="text-[13px] text-zinc-500">No providers yet.</p>
        ...
        providers.map((p) => (
          <ProviderRow key={p.id} onEdit={(p) => setMode({ kind: "edit", provider: p })} provider={p} />
        ))
      )}
    </div>
  );
