...
export type GuestNetSummary = {
  ...
  type: "lxc" | "qemu";
  running: boolean;
  /** Raw `netN=…` config strings, keyed by "net0", "net1", … */
  netSpecs: Record<string, string>;
};

/**
 * Lean guest inventory for the virtual-switching view: guest lists per node
 * plus exactly ONE config read per guest to pull the `netN` lines. Unlike
 * `getDeploymentIndex` this never touches the `/interfaces` runtime endpoint
 * (1-2s per DHCP guest) — the switching view only needs configured state.
 */
export async function listGuestNetSummaries(): Promise<{
  guests: GuestNetSummary[];
  nodes: LiveNode[];
  issues: ProxmoxIssue[];
}> {
  const { issues, nodes } = await getLiveNodeIndex();
  ...
  const listed = await Promise.all(
    nodes.map(async (node) => {
      ...
      if (lxc.issue) issues.push(lxc.issue);
      if (qemu.issue) issues.push(qemu.issue);
      const entries: { node: string; vmid: number; type: "lxc" | "qemu"; name: string; running: boolean }[] = [];
  ...
  const guests = await Promise.all(
    listed.flat().map(async (entry): Promise<GuestNetSummary | null> => {
      const path =
        entry.type === "qemu"
          ? `/nodes/${entry.node}/qemu/${entry.vmid}/config`
          : `/nodes/${entry.node}/lxc/${entry.vmid}/config`;
      const config = await safeRequest<Record<string, unknown>>(path);
      if (!config.data) return null;
      const netSpecs: Record<string, string> = {};
      for (const [key, value] of Object.entries(config.data)) {
        if (/^net\d+$/.test(key) && typeof value === "string") {
          netSpecs[key] = value;
        }
      ...
      return {
        id: encodeDeploymentId(entry.node, entry.vmid, entry.type),
        name: entry.name,
        node: entry.node,
        vmid: entry.vmid,
        type: entry.type,
  ...
  return {
    guests: guests.filter((g): g is GuestNetSummary => g !== null),
    nodes,
    issues: dedupeIssues(issues),
  };
}
