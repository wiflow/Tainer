...
import { pullOciTemplateAction } from "@/app/docker-hub-actions";
...
import type { DockerHubTag } from "@/lib/docker-hub";
import { buildDockerOciReference, buildOciTemplateFileName } from "@/lib/oci-template";
import type { TemplateTarget } from "@/lib/proxmox";
import { useSiteBasePath } from "@/lib/use-site-path";

export function DockerImageSyncPanel({
  namespace,
  repository,
  tags,
  targets,
  ...
  const [state, formAction, isPending] = useActionState(
    pullOciTemplateAction,
    initialDockerHubActionState,
  );
  useActionTaskFeedback(state, {
    errorTitle: "Template pull failed",
    successTitle: "Template pull queued",
  });
  ...
  const referencePreview = buildDockerOciReference(
    namespace,
    repository,
    selectedTag || defaultTag,
  );
  const fileNamePreview = buildOciTemplateFileName(
    namespace,
    repository,
    selectedTag || defaultTag,
  );
    ...
    <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <Card>
        <CardHeader className="border-b border-white/5">
          ...
          <CardDescription>
            Use Proxmox&apos;s native OCI registry pull to store this image as a CT template on your selected storage.
          </CardDescription>
        </CardHeader>
      ...
      <Card className="h-fit">
        <CardHeader className="border-b border-white/5">
          ...
          <CardDescription>
            Proxmox converts the registry image into a CT template archive that shows up under CT Templates.
          </CardDescription>
        </CardHeader>
