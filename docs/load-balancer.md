...
- Plans survive Tainer restarts mid-execution and can be cancelled any time (queued moves are dropped; an in-flight migration finishes — Proxmox can't recall it).
...
| Downtime Windows | — | e.g. `22:00-06:00, 12:00-13:00`, server time |
| Poll Interval | 10 s | 5–300 |
| Threshold % Above Avg | 50% | 10–200 |
| Consecutive Polls | 3 | 1–20 |
| Cooldown | 300 s | 30–3600, per source node |
| Min Improvement % | 20% | 5–80, target must beat source score by this much |
| Max Concurrent Migrations | 1 | 1–10, cluster-wide including in-flight |
| PSI Pressure Penalty | 40 | 0–200 per stalled resource, PVE 9+ only |
| PSI Threshold % | 10% | 1–100, on the 10-second stall average |
| Act on Forecasts | off | Predictive balancing (see above) |
| Forecast Horizon | 30 min | 5–120 |
| Min Confidence % | 70% | 10–99, R² floor for acting on a forecast |
| Maintenance Nodes | — | Drained and excluded from placement |
| Excluded Nodes | — | Never a migration/placement target |
| Excluded VMIDs | — | Never migrated automatically |
