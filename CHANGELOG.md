...
- **False "Will change" diffs in node config restore preview.** Proxmox returned the same DNS object with keys in different orders on different reads, and the diff did string equality on `JSON.stringify`, so identical-but-reordered objects were flagged as changed. JSON formatting now recursively sorts object keys, and arrays of objects (network interfaces, storage pools, firewall rules) are sorted by their stable identity field (`iface`, `storage`, `pos`) so semantically-equal sections show "No change" regardless of the order Proxmox returned them in.
